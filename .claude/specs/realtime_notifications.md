...
| `settings_updated` | — | 全ページ（カテゴリ・ステータス変更時） |
...
1. `solid_cable` gem を追加し、ActionCable のアダプターとして設定
2. `ProjectChannel` を作成（`stream_from "project_#{project_id}"` でプロジェクト単位に購読）
3. モデルの `after_commit` で `ActionCable.server.broadcast` を呼び出す（`Broadcastable` concern として共通化）
4. React 側に `useProjectChannel` フックを実装し、Backlogs / Kanban に組み込む
